---
title: "Redundancy analyses (RDA)-WIFL"
...
gen.imp <- fread("./input/RDA/WIFL/wifl_breeding_pop_af.csv.gz")

# Remove pop label
gen.imp <- gen.imp %>% 
  select(-pop)
dim(gen.imp) # 34 15248840

# Read in environmental info
pred = read.csv("./input/RDA/WIFL/wifl_breeding_climate.csv", header=T)
dim(pred) # 34  8

# Check correlations and remove any MEMs with correlation > 0.7
cor(pred[, -1], method="pearson")

# remove MEM2 due to correlations
pred <- pred %>% select(-MEM2)
dim(pred)
# 34  7

# Run RDA
breeding.rda <- rda(gen.imp ~ precip + temp + Condition(MEM1:MEM5), data=pred, scale=T)
...
length(cand2) # 3029 candidate loci on RDA axis 2

ncand <- length(cand1) + length(cand2)
ncand # 3029 candidate loci
...
length(cand$snp) # 3029 candidate loci
...
# Save candidate loci
write.csv(cand, "./output/RDA/WIFL/wifl_breeding_rda_candidates.csv", row.names = F, quote=F)
```
...
gen.imp <- fread("./input/RDA/WIFL/wifl_nonbreeding_pop_af.csv.gz")

# Remove pop label
gen.imp <- gen.imp %>% 
  select(-pop)
dim(gen.imp) # 22 15067894

# Read in environmental info
pred = read.csv("./input/RDA/WIFL/wifl_nonbreeding_climate.csv", header=T)
dim(pred) # 22 6
...
# Run RDA
nonbreeding.rda <- rda(gen.imp ~ precip + temp + Condition(MEM1:MEM3), data=pred, scale=T)
...
cand1 <- outliers(load.rda[, 1], 3)
length(cand1) # 3 candidate loci on RDA axis 1
cand2 <- outliers(load.rda[, 2], 3)
length(cand2) # 20636 candidate loci on RDA axis 2

ncand <- length(cand1) + length(cand2)
ncand # 20639 candidate loci
...
cand <- cand[!duplicated(cand$snp), ]
length(cand$snp) # 20639 candidate loci
...
# Save candidate loci
write.csv(cand, "./output/RDA/WIFL/wifl_nonbreeding_rda_candidates.csv", row.names = F, quote=F)
```
