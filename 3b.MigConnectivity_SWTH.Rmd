...
#Read in polygons of breeding 
```{r}
swth_genoscape<-rast("input/MigConn/SWTH/SWTH.3x.gatk3_ldExcl.genoscape_brick.tif")
swth_genoscape<-terra::rast("~/Dropbox/BGP/SWTH/LCWG/maps/SWTH.3x.genoscape_brick.WGS84.tif")
...
```
```{r}
#target.abund <- c(1312, 5173, 5931, 53786,5199,1865,25576) #earlier numbers
target.abund <- c(1239, 3653, 4353, 52567,2535,1498,21719) #2023
...
Plot transitions
```{r}
saveRDS(swth.psi,"output/MigConn/SWTH/SWTH.psi_estimates.MC.loco-pipe.snp.RDS")

pdf("output/MigConn/SWTH/SWTH.estTrans_plot.loco-pipe.angsd.lcwg_snp.pdf")
par(mar = c(2,3,0,0))
...
## Calc NMCpop for species and populations (with and without uncertainty). calcNCM is currently not working, but reached out to Jeff.
```{r}
swth.psi<-readRDS("output/MigConn/SWTH/SWTH.psi_estimates.MC.loco-pipe.snp.RDS")
