...
all<-read.delim("ecoregions_climate_resample_wintering.all.test4.inclNA.worldclim.oldcomp.txt",sep="\t") %>% filter(population!="population") %>% mutate(species=if_else(population=="EST"|population=="PNW_INW"|population=="SW","WIFL",if_else(population=="W_E_Boreal"|population=="PacificCoast"|population=="PNW","SWTH","AMRE"))) %>% na.omit()
...
ggsave("Temp_zscore.Histogram.Sampling.adjust2.pdf")
...
ggsave("Precip_zscore.Histogram.Sampling.adjust2.pdf")
...
pdf("SWTH.temp_zscore.kde_gaussian.25bw.base_plot.worldclim.pdf")
par(bty = "l")
plot(swth_kde_t2, main = "Three Gaussian Kernel Density Estimates of Temp",
...
#create list of numerical distribution to estimate gaussian kernal dist overlap (no smoothing)
??overlap
...
#This is overlap, but not of gaussian distribution (so no)
...
ggsave("SWTH.precip.overlap.plot.pdf")
...
pdf("SWTH.precip_zscore.kde_gaussian.25bw.base_plot.worldclim.NArm_WB.pdf")
par(bty = "l")
plot(swth_kde_p3, main = "Three Gaussian Kernel Density Estimates of Precip",
...
pdf("WIFL.temp_zscore.kde_gaussian.25bw.base_plot.worldclim.pdf")
par(bty = "l")
...
#Overlap but not of gaussian kernal density (so ok)
...
ggsave("WIFL.temp_zscore.overlap.plot.pdf")

WIFL_distinctT<- 1-Woverlap_resultsT$OV
WIFL_distinctTs<- 1-Woverlap_resultsTs$OV

print(paste("WIFL distinct area, bw 0.25:", WIFL_distinctTs))
...
#Yes gaussian kernal distribution (so yes- this is guassian kernel density)
...
ggsave("WIFL.precip_zscore.overlap.plot.pdf")
...
print(paste("WIFL distinct area with smoothing:", WIFL_distinctPs))

pdf("WIFL.precip_zscore.kde_gaussian.25bw.base_plot.worldclim.pdf")
par(bty = "l")
...
pdf("AMRE.temp_zscore.kde_gaussian.25bw.base_plot.worldclim.pdf")
par(bty = "l")
...
ggsave("AMRE.temp_zscore.overlap.plot.pdf")
...
ggsave("AMRE.precip_zscore.overlap.25nbin.plot.pdf")
...
pdf("AMRE.precip_zscore.kde_gaussian.25bw.base_plot.worldclim.pdf")
par(bty = "l")
