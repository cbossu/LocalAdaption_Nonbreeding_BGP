...
## Read in polygons of breeding genetic groups
```{r}
...
```
Note: We ran this multiple times (with raster over several years). The proportions largely stayed the same even if the numbers didn't
...
AMREmc <- estStrength(originDist = originDistAMRE,
                      ...
                      nSamples = 5000,
                      verbose = 0)
AMREmc #MC estimate (mean): 0.6134 +/- (SE) 0.09929 

```


## Uses the transition probabilities to estimated network migratory connectivty strength (node-specific), what we call NMCpop in MigConnectivity
