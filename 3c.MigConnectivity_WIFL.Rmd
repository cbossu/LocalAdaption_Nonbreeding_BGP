...
#Read in polygons of breeding genoscape
```{r}
...
nonbr_abunds<-rast("~/Dropbox/BGP/Network_modeling_Mignette/ConnectivityModel/WIFL/wilfly_abundance_seasonal_nonbreeding_mean_2022.tif")
...
##Target points
```{r}
target.points <- read.delim ("~/Dropbox/BGP/WIFL_meta_clean/snakemake_loco-pipe/WGSAssign/wifl.wint128.thin_10_all.assigned_pop.REF_exclWTMT.wide_consistent.angsd.loco-pipe.txt",sep="\t") %>%
...
assign_mc2<-read.delim("~/Dropbox/BGP/WIFL_meta_clean/snakemake_loco-pipe/WGSAssign/wifl.wint128.thin_10_all.assigned_pop.REF_exclWTMT.wide_consistent.angsd.loco-pipe.txt",sep="\t")  %>% 
  #rename(Sample1=Sample) %>% 
  #arrange(Sample) %>% 
  mutate(SD=0) %>% 
  dplyr::select(EST,SW,PNW_INW) %>% as.matrix()
...
```
```{r}
wifl.psi<-readRDS("LCWG_mignette/WIFL.psi_estimates.MC.loco-pipe.exclSD0.updatedMigConn.RDS")
calcNMC(wifl.psi$psi$mean)
```
...
WIFLmc <- estStrength(originDist = originDistWIFL,
                      ...
                      nSamples = 5000,
                      verbose = 0)
WIFLmc #MC estimate (mean): 0.1201 +/- (SE) 0.07217 
```